...
public class CoachDownloadService extends IntentService {
    ...
    MySql dbHelper;
    String id, sync_flag, update_table, audio_base_url, audio_title, audio_sub_url, created_at, updated_at, audio_description, download_status, sd_card_path, audio_number;

    int audio_number_int;
    ...
    protected void onHandleIntent(Intent intent) {
        ...
        try {
            ...
            audio_base_url = "https://api.nsmiles.com/v2/G_download/common/";
        ...
        /*Getting Single file and url*/
            if (intent.hasExtra("URL") && (intent.hasExtra("FILE_NAME") && intent.hasExtra("AUDIO_BASE_URL") && intent.hasExtra("AUDIO_NUMBER") && intent.hasExtra("UPDATE_TABLE"))) {
               ...
               // audio_number_int = Integer.parseInt(intent.getStringExtra("AUDIO_NUMBER"));
                update_table = intent.getStringExtra("UPDATE_TABLE");
    ...
    private void getDownloadedDataFromDB() {
        ...
        Cursor cursor = db.rawQuery("SELECT * FROM relax_coach_others WHERE DOWNLOAD_STATUS=?", new String[]{"1"});
        ...
        if (cursor.getCount() > 0) {
            ...
            do {
                ...
                if (file.exists()) {
                    ...
                    Log.i("CoachDownloadService", "Delete is " + delete);
                    if (delete) {
    ...
    private void initDownload() throws MalformedURLException {
    ...
    private void downloadFile(long sizeOfFile, InputStream ip) throws IOException {
            try {
                dbHelper = new MySql(context, "mydb", null, MySql.version);
                db = dbHelper.getWritableDatabase();
                ...
                InputStream bis = new BufferedInputStream(ip, 1024 * 8);
                ...
                // deleting dir for deleting the previous audio 0files
                if (!dir.exists()) {
                    dir.mkdirs();
                }
                ...
                db.update(update_table, cv, "AUDIO_NUMBER=?", new String[]{audio_number});
                onDownloadComplete();
                output.flush();
                output.close();
                bis.close();
    ...
    private void sendIntent(AudioDownload download) {
        ...
        try {
            ...
            intent.putExtra("audio_number", download);
            LocalBroadcastManager.getInstance(CoachDownloadService.this).sendBroadcast(intent);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
